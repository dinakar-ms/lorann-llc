...
export default function Navbar() {
  ...
  const [mobileOpen, setMobileOpen] = useState(false);
  const [openSubmenu, setOpenSubmenu] = useState<string | null>(null);
  // State-controlled desktop dropdown — replaces the previous CSS :hover-only
  // approach. Required because Next.js client-side navigation does not move
  // the cursor, so the old hover-driven dropdown stayed stuck open on the new
  // page until the user moved the mouse off the panel.
  ...
  // Single helper used by every link inside any desktop dropdown.
  // Closes the menu instantly so when Next.js navigates, the new page
  // renders with the menu already gone — no matter where the cursor sits.
              ...
              {NAV.map((item) => {
                ...
                return (
                  <li
                    key={item.label}
                    className="relative"
                    // Open this menu when the cursor enters the nav item.
                    // Setting it directly (not toggling) also handles moving
                    // sideways from one open menu to another.
                    onMouseEnter={() => {
                      if (item.children) setOpenDesktopMenu(item.label);
                    }}
                    // Close when the cursor leaves the nav item entirely.
                    // The dropdown panel is a DOM child of this <li>, so the
                    // 12px gap between trigger and panel doesn't fire mouseleave.
