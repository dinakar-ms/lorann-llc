...
import {
  ...
  Check, ArrowRight, RefreshCw, type LucideIcon,
} from "lucide-react";
...
export default function ValueProps({ content }: { content?: ValuePropsContent }) {
  ...
  const cards: Card[] =
    content?.cards && content.cards.length > 0
      ? content.cards.map((c, i) => {
          ...
          return {
            Icon: getIconComponent(c.iconName) || fallback.Icon,
            title: c.title || fallback.title,
            desc: c.desc || fallback.desc,
            backTitle: c.backTitle || fallback.backTitle,
            backList: c.backList && c.backList.length > 0 ? c.backList : fallback.backList,
            link: c.link || fallback.link,
            accent: c.accent || fallback.accent,
            accentBg: c.accentBg || fallback.accentBg,
          };
        })
      : DEFAULT_CARDS;

  const kicker = content?.kicker || "Why Lorann";
  const titleStart = content?.titleStart || "The data advantage that drives";
