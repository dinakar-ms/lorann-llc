...
export const metadata: Metadata = {
  title: "Flexible Delivery · Lorann LLC",
  description:
    "Get Lorann audience data in the format your stack expects — CSV, Excel, JSON, XML, REST API, SFTP, or scheduled refresh feeds, with no reformatting required.",
};

export default function FlexibleDeliveryPage() {
  return (
    <>
      {/* ===================== 1. HERO ===================== */}
      <section className="relative pt-36 pb-20 lg:pt-44 lg:pb-24 overflow-hidden radial-hero dot-grid">
        ...
        <div className="container-custom relative z-[5]">
          ...
          >
            ...
            <span className="inline-flex items-center gap-1.5">
              <span className="text-slate-700 font-medium">Flexible Delivery</span>
            </span>
          </nav>
          ...
          <div className="max-w-3xl">
            ...
            <p className="text-lg sm:text-xl text-slate-600 max-w-2xl leading-relaxed">
              Whether your team runs on Salesforce, HubSpot, a legacy ESP, or a custom data warehouse, we deliver audiences in the format your tools already expect — and refresh them on a cadence that matches your campaign rhythm.
            </p>
          ...
          <div className="grid lg:grid-cols-[1fr_1.2fr] gap-12 lg:gap-16 items-start max-w-6xl mx-auto">
            <div>
              <div className="font-mono text-[11px] font-semibold uppercase tracking-[0.14em] text-blue-700 mb-4 flex items-center gap-2">
                ...
                What Flexible Delivery Means at Lorann
              </div>
              <h2 className="font-display font-bold text-3xl lg:text-[2.6rem] leading-[1.1] tracking-[-0.028em] text-slate-900">
                Define delivery once.{" "}
                <span className="text-gradient">Run it forever.</span>
              </h2>
            </div>
            <div className="text-slate-700 text-[17px] leading-[1.75] space-y-4">
              <p>Most data engagements stall at the integration layer. Format mismatches, schema drift, missing fields, broken refresh cycles — these are the bugs that turn a working file into a quarterly fire drill.</p>
              <p>Lorann engagements lock in delivery on day one — file type, schema, frequency, destination — and then run on autopilot. Your team focuses on campaigns, not on cleaning files.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 lg:py-28 radial-stats">
        <div className="container-custom">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {[
              { icon: FileSpreadsheet, title: "Standard File Formats", body: "CSV, Excel, JSON, and XML exports — formatted to match your platform schema with custom field mapping included." },
              { icon: Webhook, title: "REST API Integration", body: "Pull audiences and signal data on demand via authenticated REST endpoints. Pagination, filtering, and webhooks supported." },
              { icon: ServerCog, title: "SFTP / Secure Transfer", body: "Encrypted file drops on your schedule, with PGP support and automatic notification on file landing." },
              { icon: Calendar, title: "Scheduled Refresh Cycles", body: "Daily, weekly, or monthly delivery schedules with delta updates — only the records that changed since last sync." },
              { icon: Database, title: "Native Platform Schemas", body: "Salesforce, HubSpot, Marketo, LinkedIn, Meta, and Google Ads schemas pre-mapped — no manual field translation." },
              { icon: Cable, title: "Versioned Exports", body: "Every delivery comes with a change log and version stamp, so audits are clean and rollbacks are instant." },
            ].map((card) => (
              <div
                key={card.title}
        ...
        <div className="container-custom">
          ...
          >
            <div className="relative w-28 h-28 lg:w-32 lg:h-32 rounded-[24px] bg-cyan-400/15 border border-cyan-400/[0.28] grid place-items-center">
              <GitMerge className="w-14 h-14 text-cyan-300" />
            </div>
            <div className="relative">
              ...
              <h3 className="font-display font-bold text-2xl lg:text-[2rem] tracking-[-0.025em] leading-tight mb-3.5">
                7–14 days from kickoff to first delivery — then it just runs.
              </h3>
              <p className="text-white/75 text-[15px] leading-relaxed max-w-2xl">
                Most engagements reach first delivery within two weeks, including custom schema mapping, QA on a sample file, and refresh cycle scheduling. After that, audiences flow into your stack on whatever cadence your campaigns demand — daily, weekly, monthly, or on-demand via API.
              </p>
            </div>
          </div>
        </div>
